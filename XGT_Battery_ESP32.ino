...
uint8_t temperatureCmd[] = { 0x33, 0x3B, 0x3, 0xC0, 0x58, 0x0, 0x0, 0xCC }; //command to get temperature of pack
uint8_t packVoltageCmd[] = { 0x33, 0x43, 0x3, 0xC0, 0x0, 0x0, 0x0, 0xCC }; //command to get pack voltage
uint8_t cellVoltagesCmds[] = { 0x33, 0x23, 0x03, 0xC0, 0x00, 0x0, 0x0, 0xCC }; //command to get cell voltages
...
bool getData() { //get required data from battery
  ...
  for (uint8_t i = 0; i < 9; i++) { //loop through 8 commands defined above to get required data
    switch (i) {
      case 0:
        {
      ...
      case 5:
        ...
        break;
      case 6:
        error += sendBattery(buffer, temperatureCmd, sizeof(temperatureCmd), &rxLength);
...
void loop() {
  if (digitalRead(testBtn) == 0) { //read the button
    ...
    if (!getData()) { //get data from battery, if this is ok should return 0. 
      sprintf(buffer, "Model %s\n\nCharge Count %d\nHealth %d%%\nCharge %d%%\nTemperature %.1f\nPack Voltage %.1f v\n", model, numCharges, battHealth, charge, temperature, packVoltage); //build big ugly char buffer with recievd data
